...
장소에 속한 메뉴와 공연을 조회합니다. 계약은
[API 명세](../../docs/API_SPEC.md) §2, §3.2, §3.3, §3.4를 따릅니다.
...
API는 #6의 구현 범위입니다. 공지·분실물 API는 아직 제공하지 않습니다.
...
| HTTP | code | 조건 |
| --- | --- | --- |
| 404 | `RESOURCE_NOT_FOUND` | 장소 또는 공연 ID에 해당하는 기본 리소스가 없음 |
...
공연 목록에 명세에 없는 query를 전달해도 `422`입니다.
...
장소·공연/번역의 404 구분, 이미지 순서·`null`, query 허용 목록과 OpenAPI
계약을 검증합니다. 공연 HTTP 테스트는 type·date filter, 엄격한 날짜 형식,
저장된 `is_live`의 응답 변환을 함께 검증합니다.
...
검사합니다. SQL을 DB에서 실행하지 않으므로 실제 `date`, `seq`, `is_live` 열의
존재, DB 제약과 런타임 query 실행은 Customer 테스트가 검증하지 않습니다.
