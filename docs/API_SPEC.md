...
Backoffice는 단일 공유 발급 코드 방식으로 인증한다. 서버가 보관한 발급
코드를 운영자가 백오피스 화면에서 입력하면 access token을 발급받고, 이후
모든 요청은 이 토큰으로 인증한다. 관리자 계정 로그인, 계정별 자격 증명,
역할과 권한 분리는 두지 않는다 (PRD §9-5 확정).

이 선택의 결과는 계약에 그대로 드러난다.

- 인증에 성공한 모든 운영자는 동일한 권한을 가지며, 토큰의 `sub`는 언제나
  `admin`이다. 요청 주체를 구분하는 필드는 어떤 응답에도 없다.
- 특정 운영자만 차단하려면 발급 코드를 교체해야 하고, 이때 모든 운영자가
  새 코드를 다시 입력해야 한다. 개별 토큰 폐기 수단은 제공하지 않는다.
- 발급된 토큰은 만료 전까지 유효하다. `jti`는 발급마다 새로 만들지만
  서버가 저장하지 않으므로 차단 목록으로 쓰지 않는다.
...
- JWT 서명 키는 발급 코드와 다른 256-bit 이상의 무작위 비밀값을
  사용한다.
- 발급 코드는 16자 이상이며 공백 문자를 포함하지 않는다. 두 조건 중
  하나라도 어기면 애플리케이션은 기동하지 않는다.
- 발급 코드는 사람이 정하지 않고 `secrets.token_urlsafe(16)` 수준의
  무작위 생성값을 쓴다. 길이 조건은 명백히 약한 값을 막을 뿐 무작위성을
  보장하지 않는다.
...
클라이언트는 `401 INVALID_TOKEN`을 받으면 발급 코드 입력 화면으로 돌아가
§4.1로 토큰을 다시 받는다. 갱신 엔드포인트는 없다.
...
발급 코드는 서버 측 비밀값이므로 클라이언트에 영속시키지 않는다. 백오피스
화면은 입력받은 코드를 §4.1의 토큰 교환에만 쓰고 즉시 폐기하며, 브라우저
저장소에는 access token만 둔다. 코드를 저장하면 만료 없는 발급 권한을
클라이언트에 두는 것과 같아 토큰의 수명 제한이 의미를 잃는다.

#### 비밀값 교체

**신규 발급 차단과 기존 토큰 폐기는 서로 다른 조치다.** JWT는 서버에 상태를
두지 않으므로, 발급 코드를 바꿔도 이미 서명된 토큰은 `exp`까지 유효하다.
폐기 목록(denylist)은 두지 않는다.

| 목적          | 교체 대상             | 효과                                                                            |
| ------------- | --------------------- | ------------------------------------------------------------------------------- |
| 신규 발급 차단 | 발급 코드             | 옛 코드로는 `401 INVALID_CREDENTIALS`. **이미 발급된 토큰은 최대 5시간 유효**   |
| 즉시 폐기      | 발급 코드 + 서명 키   | 옛 키로 서명된 모든 토큰이 `401 INVALID_TOKEN`. 전 운영자가 새 코드로 재인증    |

**어느 쪽이든 실행 중 인스턴스의 재시작이 필요하다.** 설정은 기동 시 한 번
읽어 프로세스 수명 동안 유지하므로, 비밀값 저장소의 값만 바꾸면 이미 떠 있는
인스턴스는 옛 값을 계속 쓴다. 재시작 전까지는 신규 발급 차단조차 일어나지
않는다.

평시 교체는 발급 코드만 바꾼다. 이 경우 §4.2의 `exp`까지 최대 5시간의
지연을 허용한다.

1. 발급 코드를 교체한다.
2. 모든 실행 중 인스턴스를 재시작해 새 설정을 반영한다.
3. 운영자에게 새 발급 코드를 전달한다. 기존 토큰은 만료될 때까지 그대로 쓴다.

코드 유출이나 운영자 이탈처럼 즉시 차단이 필요하면 1번에서 JWT 서명 키를
함께 교체하고, 3번에서 즉시 재입력을 안내한다.

2번이 끝나기 전까지 옛 설정을 들고 있는 인스턴스는 옛 코드와 옛 토큰을 모두
계속 승인한다. 롤링 배포 중에는 새 인스턴스와 옛 인스턴스가 공존하므로,
**전체 교체가 끝난 시점**을 차단·폐기 완료 시점으로 본다.
...
`IMAGE`는 object key와 업로드 상태를 소유하며 `s3_key`에 unique 제약을
둔다. 하나의 object key가 하나의 기본 리소스에만 연결된다는 규칙은 각
리소스의 `image_id` unique 제약과, 이미 연결된 key를 거부하는 서버
검증이 함께 보장한다.
