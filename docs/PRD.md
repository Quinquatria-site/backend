...
- **확정**: 공연은 시작·종료 시각을 갖지 않는다. 축제 일차(`date`)와 일차 내 순서(`seq`)로만 타임라인을 구성한다. `seq`에는 unique 제약을 두지 않고 `(date, seq)` 복합 인덱스로 정렬·필터를 처리한다.
...
- **ARTIST 테이블 포함 공연 타임라인 모델·페이지** (일차·순서 기준 정렬, 백오피스 현재공연 토글)
